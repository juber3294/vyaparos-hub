...
export const Route = createFileRoute("/payments")({
  head: () => ({
    meta: [
      { title: "Payments — VyaparOS" },
      { name: "description", content: "Received & paid entries with UPI/NEFT." },
    ],
  }),
...
function PaymentsPage() {
  ...
  const received = customers.slice(0, 8).map((c, i) => ({
    id: `RCPT-${9001 + i}`,
    party: c.name,
    mode: methods[i % methods.length],
    ref: `TXN${640125 + i * 3719}`,
    amount: 20000 + i * 15000,
  ...
  const paid = suppliers.slice(0, 6).map((s, i) => ({
    id: `PAY-${9101 + i}`,
    party: s.name,
    mode: methods[(i + 2) % methods.length],
    ref: `NEFT${510840 + i * 4283}`,
    amount: 45000 + i * 22000,
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            label: "Type",
            render: (r) => (
              <Badge tone={r.type === "IN" ? "success" : "destructive"}>
                {r.type === "IN" ? "Received" : "Paid"}
              </Badge>
            ),
          },
          {
            ...
            render: (r) => (
              <span
                className={`font-semibold ${r.type === "IN" ? "text-success" : "text-destructive"}`}
              >

...
export const Route = createFileRoute("/settings")({
  head: () => ({ meta: [{ title: "Settings — VyaparOS" }, { name: "description", content: "Company, GST, invoice & user settings." }] }),
      ...
      <Card className="p-6 mt-6">
        ...
        {[
          ...
          <div key={String(label)} className="flex items-center justify-between py-3 border-b border-border last:border-0">
            ...
            <div className={`h-6 w-11 rounded-full p-0.5 transition ${on ? "bg-primary" : "bg-muted"}`}>
              <div className={`h-5 w-5 rounded-full bg-white shadow transition ${on ? "translate-x-5" : ""}`} />
            </div>
          </div>
        ))}
      </Card>

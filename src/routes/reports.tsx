...
export const Route = createFileRoute("/reports")({
  head: () => ({ meta: [{ title: "Reports — VyaparOS" }, { name: "description", content: "GST, sales, purchase & profit reports." }] }),
...
function ReportsPage() {
  return (
    <AppShell title="Reports" subtitle="One-click statutory & business reports" actions={<Btn><Download className="h-4 w-4" /> Export All</Btn>}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {reports.map(r => (
          <Card key={r.title} className="p-5 hover:-translate-y-0.5 transition cursor-pointer">
            <div className="grid h-10 w-10 place-items-center rounded-xl bg-primary/10 text-primary"><FileBarChart className="h-5 w-5" /></div>
            <div className="mt-3 font-semibold">{r.title}</div>
            <div className="text-xs text-muted-foreground mt-1">{r.desc}</div>
            <div className="mt-4 text-xs text-primary font-semibold inline-flex items-center gap-1">View report <ChevronRight className="h-3 w-3" /></div>
          </Card>
        ))}
      </div>
    </AppShell>

...
export const Route = createFileRoute("/customers")({
  head: () => ({
    meta: [
      { title: "Customers — VyaparOS" },
      { name: "description", content: "Customer ledger, credit limits, GSTIN." },
    ],
  }),
...
function CustomersPage() {
  ...
  return (
    <AppShell
      title="Customers"
      subtitle="Ledger, credit & GST details"
      actions={
        <Btn>
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          ...
          value={inr(customers.reduce((s, c) => s + c.creditLimit, 0))}
          icon={CreditCard}
          tone="info"
        />
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: "creditLimit", label: "Credit Limit", render: (c) => inr(c.creditLimit) },
          {
            ...
            render: (c) => (
              <span
                className={`font-semibold ${c.outstanding > 100000 ? "text-destructive" : c.outstanding > 0 ? "text-warning" : "text-success"}`}
              >
                {inr(c.outstanding)}
              </span>
            ),
            ...
            render: (c) => (
              <Badge
                tone={
                  c.outstanding === 0
                    ? "success"
                    : c.outstanding > c.creditLimit * 0.7
                      ? "destructive"
                      : "info"
                }
              >

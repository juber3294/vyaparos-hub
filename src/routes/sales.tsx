...
export const Route = createFileRoute("/sales")({
  head: () => ({
    meta: [
      { title: "Sales — VyaparOS" },
      { name: "description", content: "Create GST invoices and manage sales." },
    ],
  }),
  component: SalesPage,
});
...
function SalesPage() {
  const preview = {
    ...
    items: products.slice(0, 4).map((p, i) => ({ ...p, qty: 2 + (i % 5) })),
  };
  ...
  return (
    <AppShell
      ...
      actions={
        <>
          <Btn variant="outline">
            <Receipt className="h-4 w-4" /> Templates
          </Btn>
          <Btn>
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-1">
                Dated 12 Jul 2026 · Place of supply: Maharashtra (27)
              </div>
            </div>
        ...
        <div className="space-y-4">
          <Card className="p-5">
            ...
            <div className="mt-4 text-xs text-muted-foreground">
              Received via UPI (rajesh@okhdfc)
            </div>
          </Card>
          <Card className="p-5">

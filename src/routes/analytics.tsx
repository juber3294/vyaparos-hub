...
import { AppShell, Card, StatCard, Badge, type BadgeTone } from "@/components/app-shell";
...
export const Route = createFileRoute("/analytics")({
  head: () => ({
    meta: [
      { title: "Analytics — VyaparOS" },
      { name: "description", content: "Business growth, margin & product analytics." },
    ],
  }),
...
function AnalyticsPage() {
  const growth = monthlySales.map((m) => ({
    m: m.m,
    growth: Math.round(((m.sales - m.purchase) / m.sales) * 100),
  }));
    ...
    <AppShell title="Analytics" subtitle="Understand what's really happening in your business">
        ...
        <Card className="p-6">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <RadialBarChart
              innerRadius="65%"
              outerRadius="100%"
              data={[{ name: "score", value: 82, fill: "#2563EB" }]}
              startAngle={220}
              endAngle={-40}
            >
          ...
          <div className="mt-16 space-y-2 text-xs">
            {(
              [
                ["Liquidity", "Strong", "success"],
                ["Receivables ageing", "Watch", "warning"],
                ["Inventory turn", "Healthy", "success"],
              ] satisfies [string, string, BadgeTone][]
            ).map(([k, v, t]) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        <Card className="p-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold">Fast-Moving Products</span>
          </div>
        ...
        <Card className="p-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold">Most Profitable Products</span>
          </div>
          <ul className="space-y-2 text-sm">
      ...
      <Card className="p-6 mt-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-semibold">Dead Stock — no movement in 90 days</span>
        </div>
          ...
          {dead.map((p) => (
            <div key={p.id} className="rounded-xl border border-border p-3">
              ...
              <div className="text-xs text-muted-foreground">
                {p.stock} units · {inr(p.stock * p.purchasePrice)} blocked
              </div>
            </div>
          ))}
        </div>

...
export const Route = createFileRoute("/employees")({
  head: () => ({ meta: [{ title: "Employees — VyaparOS" }, { name: "description", content: "Team, roles and permissions." }] }),
  component: EmployeesPage,
});

function EmployeesPage() {
  return (
    <AppShell title="Employees" subtitle="Team members & role-based access" actions={<Btn><UserPlus className="h-4 w-4" /> Add Member</Btn>}>
      ...
      <DataTable
        rows={employees}
        columns={[
          { key: "name", label: "Member", render: (e) => (
            <div className="flex items-center gap-3">
              ...
              <div><div className="font-medium">{e.name}</div><div className="text-[11px] text-muted-foreground">{e.phone}</div></div>
            </div>
          )},
          { key: "role", label: "Role", render: (e) => <Badge tone="primary">{e.role}</Badge> },
          { key: "dept", label: "Department" },
          { key: "id", label: "Employee ID", render: (e) => <span className="font-mono text-xs">{e.id}</span> },

...
export const Route = createFileRoute("/inventory")({
  head: () => ({
    meta: [
      { title: "Inventory — VyaparOS" },
      { name: "description", content: "Product master with GST, HSN & stock." },
    ],
  }),
...
function InventoryPage() {
  ...
  return (
    <AppShell
      title="Inventory"
      subtitle="44 SKUs across 3 warehouses"
      actions={
        <Btn>
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: "purchasePrice", label: "Purchase", render: (p) => inr(p.purchasePrice) },
          {
            key: "sellingPrice",
            label: "Selling",
            render: (p) => <span className="font-semibold">{inr(p.sellingPrice)}</span>,
          },
          {
            key: "stock",
            label: "Stock",
            render: (p) => (
              <span
                className={`font-semibold ${p.stock < p.minStock ? "text-destructive" : p.stock < 30 ? "text-warning" : "text-success"}`}
              >
                {p.stock}
              </span>
            ),

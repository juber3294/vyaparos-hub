...
export const topCustomers = customers.slice(0, 6).map((c, i) => ({
  name: c.name.split(" ").slice(0, 2).join(" "),
  value: Math.floor(180000 - i * 22000),
}));

export const topProducts = products.slice(0, 6).map((p, i) => ({
  name: p.name,
  sold: Math.floor(340 - i * 40),
  revenue: Math.floor(240000 - i * 28000),
}));
